...
prompt_migration() {
  ...
  ### CHANGE: Forced migration. If user closes the dialog, we still proceed.
...
renew_profiles() {
  ### CHANGE: Keep this as-is, but we will call it earlier in the main flow for ADE.
...
unmanage_device_jamf_new() {
    ...
    if echo "$response" | jq -e '.commandUuid' >/dev/null; then
        ...
        ### CHANGE (B): Do NOT remove Jamf framework here. We remove it later after Intune MDM is present.
...
wait_for_management_profile_removal() {
  ...
  while true; do
    ### CHANGE (C): Fix bug. Correct command syntax is: profiles show -type enrollment
    local output
    output=$(profiles show -type enrollment 2>/dev/null)

    # If the MDM payload is missing, consider enrollment profile removed.
...
wait_for_intune_mdm_presence() {
  ...
  while true; do
    ...
    # We only need com.apple.mdm to re-appear (post-Jamf removal).
    if echo "$output" | grep -q "com.apple.mdm"; then
...
if [ "$ADE_ENROLLED" = true ]; then
    ...
    ### CHANGE (E): Opening Company Portal isn't enough. Launch CP, then WAIT for MDM to appear.
    cp_sign_in_message
    launch_company_portal
    waiting_for_intune
