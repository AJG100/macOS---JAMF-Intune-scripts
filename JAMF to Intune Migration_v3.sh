...
# V2 includes improvements with typos and the Profile removal dependencies - Ariel Juarez - January 15th 2026
# V3 includes a 5 minute times to avoid early exit from the Waiting for Intune prompt - Ariel Juarez - January 15th 2026
# -------------------------------------------------------------------------------------------------------
# Dependencies:
...
JAMF_PRO_URL="https://johnsoncontrols.jamfcloud.com"  # URL of your Jamf Pro server
USERNAME="intune-migration"                          # This should be a Jamf Pro user with the Jamf Pro Server Action 'Send Computer Unmanage Command' enabled and Jamf Pro Server Objects 'Computers' Read.
PASSWORD="SmoothM!gr@tion"                           # Password for the above user
...
# ### CHANGE: Replaced with enforced 5-minute timer dialog (OK disabled until 5 minutes).
waiting_for_intune() {

  COMMAND_FILE="/tmp/dialog_intune_wait"
  COUNTDOWN=300   # 5 minutes in seconds

  # Initial state: button disabled
  echo "button1: disabled" > "$COMMAND_FILE"
  echo "progresstext: Preparing enrollment..." >> "$COMMAND_FILE"

  /usr/local/bin/dialog \
    --bannertitle "Status: Preparing Intune Enrollment" \
    --message "Your device setup is in progress.\n\nPlease wait while enrollment stabilizes.\n\nYou will be able to continue shortly." \
    --button1text "OK" \
    --blurscreen \
    --bannerimage colour=blue \
    --titlefont shadow=1 \
  ...
  while [ $COUNTDOWN -gt 0 ]; do
    MIN=$((COUNTDOWN / 60))
    SEC=$((COUNTDOWN % 60))
    printf "progresstext: Please wait %02d:%02d before continuing...\n" "$MIN" "$SEC" > "$COMMAND_FILE"
    sleep 1
    COUNTDOWN=$((COUNTDOWN - 1))
  done

  # Enable the button after countdown
  echo "progresstext: You may now continue.\n" > "$COMMAND_FILE"
...
prompt_migration() {
  ...
  # Display the dialog (FORCED: no Exit button)
  /usr/local/bin/dialog \
  ...
  # Forced migration - proceed regardless
  echo "Proceeding with migration (forced)."
  return 0
}
...
unmanage_device_jamf_new() {
    ...
    if echo "$response" | jq -e '.commandUuid' >/dev/null; then
        ...
        # CHANGE (B): Do NOT remove Jamf framework here.
...
wait_for_management_profile_removal() {
  ...
  while true; do
    # CHANGE (C): Fix bug. Correct command syntax:
    local output
    output=$(profiles show -type enrollment 2>/dev/null)

    if ! echo "$output" | grep -q "com.apple.mdm"; then
...
# CHANGE (B/E): Wait for Intune MDM to be present after enrollment trigger
wait_for_intune_mdm_presence() {
  echo "Waiting for Intune MDM enrollment profile to be present..."
  local timeout=1800
  local interval=10
  local elapsed=0
  ...
  while true; do
    ...
    if echo "$output" | grep -q "com.apple.mdm"; then
      echo "MDM enrollment profile detected."
      break
    else
      echo "MDM not detected yet. Retrying in ${interval} seconds..."
    fi

    sleep "${interval}"
    elapsed=$((elapsed + interval))
    if [ $elapsed -ge $timeout ]; then
      echo "Timeout waiting for Intune MDM enrollment profile." >&2
      exit 1
    fi
  done
}
...
# Wait for Jamf MDM profile removal
wait_for_management_profile_removal

# ADE vs non-ADE flows
if [ "$ADE_ENROLLED" = true ]; then
    # CHANGE (D): Renew enrollment immediately, then message + wait for MDM
    echo "ADE device detected. Renewing enrollment immediately..."
    renew_profiles

    ade_enrollment_message
    sleep 5

    # Enforced 5-minute wait (OK disabled)
    waiting_for_intune

    wait_for_intune_mdm_presence
else
    # CHANGE (E): Open CP, then enforced wait, then WAIT for MDM to appear.
    cp_sign_in_message
    launch_company_portal
...
# CHANGE (B): Only remove Jamf framework AFTER Intune MDM is present
remove_jamf_framework
